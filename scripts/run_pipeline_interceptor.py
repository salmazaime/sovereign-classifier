...
from app.interceptor.aws_identity import resolve_current_aws_account_id
...
def main() -> int:
    ...
    # Resolve the AWS account this pipeline run is currently
    # authenticated as, if any credentials are configured. Never
    # raises -- returns None and every AWS resource keeps its
    # "unknown" default when no credentials are present, which is
    # the normal case for repos where a separate, credentialed step
    # handles the actual deploy.
    aws_account_id = resolve_current_aws_account_id()
