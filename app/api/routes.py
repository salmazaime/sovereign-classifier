...
from app.compliance.pdf_report import generate_evidence_pack_pdf
...
from app.policy.explain import explain_review_reasoning
...
@router.get("/policy-decisions/{policy_decision_id}/infrastructure", tags=["compliance"])
@router.get("/policy-decisions/{policy_decision_id}/infrastructure", tags=["compliance"])
def get_decision_infrastructure(
    policy_decision_id: UUID,
    user: AuthenticatedUser = Depends(get_current_user),
    postgres_repo: PostgresRepository = Depends(get_postgres_repo),
) -> dict:
    row = postgres_repo.get_decision_infrastructure_detail(policy_decision_id)
    if row is None:
    ...
    return {
        ...
        "content_findings": row["content_findings"] or [],
        "pushed_by": row["initiated_by"],
        "pushed_via": row["initiating_application"],
        "pushed_at": row["requested_at"].isoformat() if row["requested_at"] else None,
        "operation": row["operation"],
        "destination_country": row["destination_country"],
    }
...
@router.get("/compliance/evidence-packs/{pack_id}/document", tags=["compliance"])
def download_evidence_pack_pdf(
    pack_id: UUID,
    ...
    records = postgres_repo.get_evidence_pack_full_records(pack_id)
    if not records:
        raise HTTPException(status_code=404, detail="Evidence pack not found or empty.")

    company = postgres_repo.get_company_profile(UUID(user.company_id))

    # Every timestamp field needs to become a plain string before
    # reaching fpdf -- datetime objects aren't directly renderable,
    # and this is also where any remaining None values get normalized
    # so _safe() in the PDF generator has a consistent input shape.
    decisions = []
    for r in records:
        decisions.append({
            "entity_name": r["entity_name"],
            ...
            "original_law_article": r["original_law_article"],
            "original_reason_code": r["original_reason_code"],
            "reviewer_name": r["reviewer_name"],
            "reviewer_email": r["reviewer_email"],
            "resolved_at": r["resolved_at"].isoformat() if r["resolved_at"] else None,
            "cndp_reference": r["cndp_reference"],
        })

    pdf_bytes = generate_evidence_pack_pdf(
        company_name=company["name"], period_start=period_start, period_end=period_end,
        decisions=decisions,
    )
    return Response(
        content=pdf_bytes, media_type="application/pdf",
        headers={"Content-Disposition": f"attachment; filename=evidence_pack_{pack_id}.pdf"},
    )
    
@router.get("/companies/{company_id}/geo-distribution", tags=["compliance"])
def get_geo_distribution(
    ...
    if str(company_id) != user.company_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Cannot view another company's data.")
    rows = postgres_repo.get_geo_distribution(company_id)
    return [
        {"country": r["country"], "cloud": r["cloud"], "decision": r["decision"], "count": r["decision_count"]}
        for r in rows
    ]
    


@router.get("/policy-decisions/{policy_decision_id}/review-reasoning", tags=["compliance"])
def get_review_reasoning(
    policy_decision_id: UUID,
    user: AuthenticatedUser = Depends(get_current_user),
    postgres_repo: PostgresRepository = Depends(get_postgres_repo),
) -> dict:
    context = postgres_repo.get_review_reasoning_context(policy_decision_id)
    if context is None:
        raise HTTPException(status_code=404, detail="Decision not found.")
    return explain_review_reasoning(context)
