...
def _sample_content_findings_locally(s3_client, bucket_name: str) -> list[dict]:
    ...
    for obj in objects:
        key = obj["Key"]
        if requires_manual_review(key):
            findings.append({
                "category": "ordinary_pii",  # conservative default category
                "field_or_location": key,
                "confidence": 0.3,
                "detector": "unscannable_format_flagged_for_review",
            })
            logger.warning("Object %s is an unscannable format (video/executable/archive) -- flagged for manual review, not content-scanned.", key)
            continue
