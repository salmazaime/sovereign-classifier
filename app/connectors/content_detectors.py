...
_RELIGIOUS_KEYWORDS = re.compile(
    r"\b(musulman|chr[ée]tien|juif|religion|croyance religieuse|pratiquant|"
    r"islam|christianisme|juda[iï]sme)\b", re.IGNORECASE
)
_POLITICAL_KEYWORDS = re.compile(
    r"\b(parti politique|opinion politique|adh[ée]sion syndicale|"
    r"affiliation politique|militant)\b", re.IGNORECASE
)
_UNION_KEYWORDS = re.compile(
    r"\b(syndicat|adh[ée]sion syndicale|d[ée]l[ée]gu[ée] syndical|CDT|UMT|UGTM)\b",
    re.IGNORECASE
)
_HEALTH_KEYWORDS = re.compile(
    r"\b(dossier m[ée]dical|diagnostic|maladie|traitement m[ée]dical|"
    r"num[ée]ro s[ée]curit[ée] sociale|CNSS|groupe sanguin|handicap)\b",
    re.IGNORECASE
)
_CRIMINAL_KEYWORDS = re.compile(
    r"\b(casier judiciaire|condamnation|infraction|antécédent judiciaire|"
    r"peine de prison)\b", re.IGNORECASE
)
_RACIAL_ETHNIC_KEYWORDS = re.compile(
    r"\b(origine ethnique|origine raciale|amazigh|arabe|berb[èe]re)\b",
    re.IGNORECASE
)

# --- Formats we cannot meaningfully content-scan today ---
# Videos need frame extraction + OCR per frame (real, multi-hour
# engineering effort). Executables need disassembly/static analysis
# (a different discipline entirely -- malware/binary analysis, not
# DLP). Rather than silently pass these through unscanned, we flag
# them explicitly -- fail-cautious, same principle as every other
# "unknown -> assume risk" default in this project since Step 13.
UNSCANNABLE_EXTENSIONS = {
    ".mp4", ".mov", ".avi", ".mkv", ".webm",   # video
    ".exe", ".dll", ".bin", ".sh", ".msi",      # executables
    ".zip", ".rar", ".7z", ".tar", ".gz",       # archives (contents unknown without extraction)
}


def requires_manual_review(filename: str) -> bool:
    return any(filename.lower().endswith(ext) for ext in UNSCANNABLE_EXTENSIONS)
...
def detect_content_findings(text: str) -> list[dict]:
    """
    Checks every SensitivityCategory the legal schema models, not
    just the three with easy structural patterns.
    """
    findings: list[dict] = []

    def _add(category: str, confidence: float):
        findings.append({
            "category": category, "field_or_location": "sampled_content",
            "confidence": confidence, "detector": "regex",
        })

    if _CIN_PATTERN.search(text):
        _add("national_id", 0.6)
    if _EMAIL_PATTERN.search(text):
        _add("ordinary_pii", 0.9)
    if _PHONE_PATTERN.search(text):
        _add("ordinary_pii", 0.75)
    if _HEALTH_KEYWORDS.search(text):
        _add("health", 0.65)
    if _RELIGIOUS_KEYWORDS.search(text):
        _add("religious_philosophical", 0.55)
    if _POLITICAL_KEYWORDS.search(text):
        _add("political", 0.55)
    if _UNION_KEYWORDS.search(text):
        _add("union", 0.6)
    if _CRIMINAL_KEYWORDS.search(text):
        _add("criminal_record_or_security_measure", 0.6)
    if _RACIAL_ETHNIC_KEYWORDS.search(text):
        _add("racial_ethnic_origin", 0.5)
