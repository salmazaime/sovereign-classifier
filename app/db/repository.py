...
class PostgresRepository:
    ...
    def get_company_profile(self, company_id: UUID) -> Optional[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT id, name, sector, is_oiv, oiv_sector, qualified_provider_required
    ...
    def create_user_with_role(
        self, company_id: UUID, name: str, email: str, password_hash: str, role_name: str
    ) -> UUID:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                ...
                if role is None:
                    raise ValueError(f"Unknown role: {role_name}")
                cur.execute(
                    "INSERT INTO USER_ROLE (user_id, role_id) VALUES (%s, %s) ON CONFLICT DO NOTHING;",
                    (str(user_id), role["id"]),
                )
                conn.commit()
                return user_id

    def list_company_users(self, company_id: UUID) -> list[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
    ...
    def list_company_api_keys(self, company_id: UUID) -> list[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT id, name, revoked, created_at, last_used_at
                    FROM API_KEY WHERE company_id = %s ORDER BY created_at DESC;
                    """,
                    (str(company_id),),
                )
                return cur.fetchall()

    def revoke_api_key(self, api_key_id: UUID, company_id: UUID) -> bool:
        with self._pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE API_KEY SET revoked = true WHERE id = %s AND company_id = %s;",
                    (str(api_key_id), str(company_id)),
                )
                conn.commit()
                return cur.rowcount == 1


    def get_evidence_pack_full_records(self, pack_id: UUID) -> list[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT
                        pd.id AS policy_decision_id,
                        pd.decision, pd.decided_at, pd.model_name,
                        pd.decision_features,
                        e.id AS entity_id, e.name AS entity_name,
                        tr.destination_country,
                        lc.article_number AS law_article,

                        orig.decision AS original_decision,
                        orig.decided_at AS original_decided_at,
                        orig_lc.article_number AS original_law_article,
                        (orig.decision_features->>'reason_code') AS original_reason_code,

                        ar.reviewed_by AS reviewer_user_id,
                        reviewer.name AS reviewer_name,
                        reviewer.email AS reviewer_email,
                        ar.decision_at AS resolved_at,
                        ar.cndp_reference

                    FROM COMPLIANCE_EVIDENCE_ITEM cei
                    JOIN POLICY_DECISION pd ON cei.policy_decision_id = pd.id
                    JOIN ENTITY e ON pd.entity_id = e.id
                    LEFT JOIN TRANSFER_REQUEST tr ON pd.transfer_request_id = tr.id
                    LEFT JOIN CLASSIFICATION_EVIDENCE ce ON ce.policy_decision_id = pd.id
                    LEFT JOIN LAW_CLAUSE lc ON ce.law_clause_id = lc.id
                    ...
                    LEFT JOIN LATERAL (
                        SELECT p2.decision, p2.decided_at, p2.decision_features, p2.id
                        FROM POLICY_DECISION p2
                        WHERE p2.entity_id = pd.entity_id
                          AND p2.model_name = 'rule_based_engine'
                          AND p2.decided_at < pd.decided_at
                        ORDER BY p2.decided_at DESC
                        LIMIT 1
                    ) orig ON pd.model_name = 'human_reviewer'
                    LEFT JOIN CLASSIFICATION_EVIDENCE orig_ce ON orig_ce.policy_decision_id = orig.id
                    LEFT JOIN LAW_CLAUSE orig_lc ON orig_ce.law_clause_id = orig_lc.id
                    LEFT JOIN AUTHORIZATION_REQUEST ar
                        ON ar.policy_decision_id = orig.id AND pd.model_name = 'human_reviewer'
                    LEFT JOIN USER_ACCOUNT reviewer ON reviewer.id = ar.reviewed_by
    ...
    def get_decision_infrastructure_detail(self, policy_decision_id: UUID) -> Optional[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT
                        e.name AS entity_name, e.entity_type,
                        cs.payload->'asset'->'infrastructure' AS infrastructure,
                        cs.payload->'asset'->'content_findings' AS content_findings,
                        tr.initiated_by, tr.initiating_application, tr.requested_at,
                        tr.destination_country, tr.operation,
    ...
    def get_user_by_id(self, user_id: UUID) -> Optional[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM USER_ACCOUNT WHERE id = %s;", (str(user_id),))
                return cur.fetchone()

    def update_company_profile(
        self, company_id: UUID, sector: Optional[str], is_oiv: Optional[bool],
        oiv_sector: Optional[str], qualified_provider_required: Optional[bool],
    ) -> dict:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    ...
                    UPDATE COMPANY SET
                        ...
                        is_oiv = COALESCE(%s, is_oiv),
                        oiv_sector = COALESCE(%s, oiv_sector),
                        qualified_provider_required = COALESCE(%s, qualified_provider_required)
                    WHERE id = %s
                    RETURNING id, name, sector, is_oiv, oiv_sector, qualified_provider_required;
                    """,
                    (sector, is_oiv, oiv_sector, qualified_provider_required, str(company_id)),
                )
                row = cur.fetchone()
                conn.commit()
                return row
    ...
    def get_geo_distribution(self, company_id: UUID) -> list[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT
                        tr.destination_country AS country,
                        pd.decision_features->>'destination_cloud' AS cloud,
                        pd.decision,
                        count(*) AS decision_count
                    FROM POLICY_DECISION pd
                    JOIN TRANSFER_REQUEST tr ON pd.transfer_request_id = tr.id
                    WHERE pd.company_id = %s AND pd.is_current = true
                      AND tr.destination_country IS NOT NULL
                    GROUP BY tr.destination_country, cloud, pd.decision;
                    """,
                    (str(company_id),),
                )
                return cur.fetchall()
    ...
    def get_review_reasoning_context(self, policy_decision_id: UUID) -> Optional[dict]:
        with self._pool.connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    """
                    SELECT
                        pd.decision_features,
                        cs.payload->'classification' AS classification,
                        cs.payload->'asset'->'content_findings' AS content_findings,
                        c.qualified_provider_required, c.is_oiv,
                        tr.destination_country, tr.destination_deployment_type
                    FROM POLICY_DECISION pd
                    JOIN CANONICAL_SCHEMA cs ON pd.canonical_schema_id = cs.id
                    JOIN COMPANY c ON pd.company_id = c.id
